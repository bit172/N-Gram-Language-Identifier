...
def create_data_frames(unique_characters, initial_value):
    ...
    for lang, characters in unique_characters.items():
        data_frames[lang] = pd.DataFrame(data = initial_value[lang], columns=characters.keys(), index=characters.keys(), dtype=np.float64)
    return data_frames
...
def execute(input_v, input_n, input_s, input_train, input_test):
    ...
    for lang, chars in unique_characters.items():
        size = len(chars)
        two_d_arrs[lang] = np.full((size, size), s_factor, dtype=np.float64)
    ...
    for lang, tweets in training_tweets.items():
        for tweet in tweets:
            for i in range(len(tweet) - 1):
                ...
                two_d_arrs[lang][c1_idx][c2_idx] += 1

    # data_frames =  create_data_frames(unique_characters, two_d_arrs)
    #
